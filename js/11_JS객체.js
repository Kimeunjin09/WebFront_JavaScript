...
document.getElementById("btn1").addEventListener("click", function() {
    ...
    // -> key 작성시, "", '', (따옴표X==따옴표미작성)든 모두 string 인식
    ...
    const product = {
        ...
        'information' : function() { // 객체의 기능
            ...
            // 왜? 같은 객체의 변수(key)를 선택하는 방법이 별도 존재해서(this.)

            // this.brand 찍으면 스타벅스 나옴 this는 현재 객체를 의미하니까
            // product의 brand가 나옴
    ...
    div1.innerHTML = ""; // 선택할때마다 쌓이면 안되니까 이전내용 삭제
...
document.getElementById("btn2").addEventListener("click", function() {
    ...
    stdList.push(new Student(2, 7, '이길동'));
    stdList.push(new Student(1, 6, '박길동'));

    div2.innerHTML = "";

    // div2 박스에 띄우기
    // for of : 배열/컬렉션 요소 반복 접근용 for문
    for(let std of stdList) { // std안에 stdList 값 저장(index하나씩 접근)

        // 객체 안의 key, value값도 얻어와야함
        // for in : 객체의 키 반복 접근용 for문
        for(let key in std) {

            // std[key]의(grade, ban, name, introduce) 자료형이 function 이면 실행 후 출력
        ...
        div2.innerHTML += "<hr>"; // 객체 끝날때마다 hr 하나씩 긋기
    }
})
...
function Student(grade, ban, name) {
    ...
    // 속성
    this.grade = grade; // 여기의 grade를 this.grade에 대입
    // 매개변수로 전달받은 grade 값을 생성되는 객체의 grade에 대입
    this.ban = ban;
    this.name = name;
