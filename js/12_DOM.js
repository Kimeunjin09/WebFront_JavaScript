...
document.getElementById("btn1").addEventListener("click", function() {
    ...
    console.log(test.childNodes[5].previousSiblig); // 4번

    // #test의 자식 노드 중 5번째 인덱스의 다음 형제 노드
    console.log(test.childNodes[5].nextSibling); // 6번

    // 노드 탐색 구문 연속 작성
    // #test의 자식 노드 중 5번째 인덱스의 이전 형제 노드의 이전형제 노드
...
btn4.addEventListener("click", function() {
    if(count4 <= 10) {
        ...
        div.innerText = count4; // 옆에 숫자가 붙도록
        count4++;

        // input을 div의 마지막 자식으로 추가
        div.append(input); // 노란색박스안에 인풋까지 넣어진상태
